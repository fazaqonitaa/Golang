...
func getCollege(w http.ResponseWriter, r *http.Request) {
	...
	sql := `select
				...
				FROM
				college.college
				INNER JOIN college.fakultas
				ON (college.Id_Fakultas = fakultas.id_fakultas)
				INNER JOIN college.jurusan
				ON (college.Id_Jurusan = jurusan.id_jurusan) where college.id_mahasiswa=?`
	...
	for result.Next() {
		err := result.Scan(&college.Idmahasiswa, &college.Nama, &college.Fakultas, &college.Jurusan,
			&college.Alamat.Jalan, &college.Alamat.Kelurahan, &college.Alamat.Kecamatan, &college.Alamat.Kabupaten, &college.Alamat.Provinsi)

		Idmahasiswa := &college.Idmahasiswa
		...
		for resultnilai.Next() {
			...
			college.Nilai = append(college.Nilai, nilaidet)
		}
...
func main() {
	...
	r.HandleFunc("/mahasiswa/{id}", updatecollege).Methods("PUT")
	r.HandleFunc("/mahasiswaT", createcollege).Methods("POST")
	r.HandleFunc("/mahasiswa", getcollege).Methods("GET")
